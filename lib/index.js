...
export function apply(ctx, config = {}) {
  ...
  if (repairOnStart) {
    const repair = async () => {
      ...
      // DSH 0.1.5's projection-cache reads are cut-addressed:
      //   cachedSnapshot(meta: SessionHeader, inheritedEventCount, keys?)
      //   coldSnapshot(meta: SessionHeader, inheritedEventCount, events)
      // Both REJECT a bare id / bare header — identityOf() throws
      // "SessionLogOffset must be a non-negative safe integer, got undefined"
      // when the cut is missing. The exact cut is not on the listing entry;
      // it lives on the log read handle, so open each log once and reuse it
      // for both the cache probe and the cold fold.
      const abort = new AbortController()
      const openable = typeof persistence.open === 'function'
      if (!openable) {
        stats.lastError = 'sessionPersistence.open is not a function; startup self-heal inactive'
        ctx.logger?.warn?.(`${name}: ${stats.lastError}`)
        return
      }
      for (const entry of headers) {
        // 0.1.5 returns SessionPersistenceSnapshot ({header, revision, ...}) instead of
        // SessionHeader; 0.1.1 returns bare headers. Accept both shapes.
        ...
        let handle
        try {
          handle = await persistence.open(header.id, 'read', { signal: abort.signal })
        } catch (error) {
          // Missing log / abandoned session — expected, keep going.
          stats.lastError = `open failed for ${header.id}: ${error instanceof Error ? error.message : String(error)}`
          continue
        }
        try {
          const meta = handle.header ?? header
          const cut = handle.inheritedEventCount
          let cached
          try {
            cached = cache.cachedSnapshot(meta, cut)
          } catch (error) {
          ...
          let events = []
          try {
            const read = await handle.read(0, undefined, { signal: abort.signal })
            if (Array.isArray(read?.events)) events = read.events
          } catch (error) {
            stats.lastError = `read failed for ${header.id}: ${error instanceof Error ? error.message : String(error)}`
            continue
          }
          try {
            cache.coldSnapshot(meta, cut, events)
